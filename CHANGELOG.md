...
### Changed
- Snapshot saves now serialize JSON incrementally through a 64 KiB spooled temporary file instead of materialising the complete JSON string and UTF-8 byte string in memory. Optional `max_bytes` limits are enforced while encoding, and the destination is opened only after serialization succeeds, so overflow and serialization errors preserve an existing checkpoint.
