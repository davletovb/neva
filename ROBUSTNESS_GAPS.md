...
- Built-in text tools now declare schemas on this branch: calculator, Wikipedia, summarizer, and translator accept the existing `input`/`task`/`query`/`text` aliases and reject unknown or non-text-only payloads before execution.
