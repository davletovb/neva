...
Updated against `main` at `f8e24bf` (PRs #49–#52, #54–#62 merged).
...
- Built-in text tools now declare schemas on this branch: calculator, Wikipedia, summarizer, and translator mirror the existing payload normalizer (`input`/`task`/`query`/`text`, metadata-bearing calls, and single-string mappings) while rejecting mapping shapes that cannot resolve to one text payload before execution.
