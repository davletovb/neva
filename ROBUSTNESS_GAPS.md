...
- PR #67 adds explicit data-only checkpoint adapters for the remaining non-FAISS first-party memory modules (`VectorStoreMemory` and `AdaptiveConversationMemory`), including adaptive budget/summary/vector state without replaying embedders or summarizers. Remaining hook-only scope is third-party/custom memory/scheduler implementations plus deferred `FaissVectorStoreMemory`/PR #53.
